import pandas as pd
from obspy import read, read_events, UTCDateTime, read_inventory, Stream
...
def trim_event(args):
    ...
    if win_max.strftime("%j") == ot.strftime("%j"):
        next_day = True
    if next_day:
        _next_day = (ot+86400).date
        files += glob("%s/%s*"%(data_path, str(_next_day)))
    ...
    if len(st) == 3:
        ...
        folder = "%s%s%s_%s%s%s.a"%(year, month, day, hour, minute, second)
        mkdir("raw/%s"%folder)
        st.write("raw/%s/%s.%s.rsbr.mseed"%(folder, net, sta), format="MSEED")
        print("Saved %s"%ot.datetime)

...
def Braunmiller_Pornsopin_algorithm(tr1,tr2,trZ,noise,baz,time_ins,CCVR_MIN=CVR_MIN,SNR_MIN=SNR_MIN,TRR_MIN=TRR_MIN,RVR_MIN=RVR_MIN):
    ...
    if (SZR_best >= CCVR_MIN) & (SNR >= SNR_MIN) & (ERTR_best < TRR_MIN) & (ERRZ_best >= RVR_MIN):
        quality = 'good'
    else:
        quality = 'bad'
...
def calculate_metrics(input_lst):
    ...
    if len(arrivals) > 0 and (gcarc < 100 or 140 < gcarc <= 180):
            ...
            station_pwd = glob.glob("%s/%s.%s/%s.%s.*%s.%s*"%(WAVE_DIR, network, station, network, station, year, julian_day))
            ...
            if os.path.isfile(file_feather_name):
                ...
                if len(files) >= 3:
                    ...
                    tr2_data_file.detrend("demean")
                    tr2_data_file.detrend("linear")
                    ...
                    tr1_data_file.detrend("demean")
                    tr1_data_file.detrend("linear")
                    ...
                    trZ_data_file.detrend("demean")
                    trZ_data_file.detrend("linear")
